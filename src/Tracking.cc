...
{
    unique_lock<std::mutex> lock(mMutexImuQueue);
...
void Tracking::Track()
{
    ...
    {
        std::cout << "TRACK: Reset std::map because local mapper set the bad imu flag " << std::endl;
    ...
    unique_lock<std::mutex> lock(pCurrentMap->mMutexMapUpdate);
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                else if (mState == LOST)
                {
                    ...
                    Verbose::PrintMess("A new std::map is started...", Verbose::VERBOSITY_NORMAL);

                    if (pCurrentMap->KeyFramesInMap()<10)
                    {
                        mpSystem->ResetActiveMap();
                        Verbose::PrintMess("Reseting current std::map...", Verbose::VERBOSITY_NORMAL);
                    }else
                        CreateMapInAtlas();
...
#endif
        // If we have an initial estimation of the camera pose and matching. Track the local std::map.
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
        ...
        {
            // mbVO true means that there are few matches to MapPoints in the std::map. We cannot retrieve
            // a local std::map and therefore we do not perform TrackLocalMap(). Once the system relocalizes
            // the camera we will use the local std::map again.
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
        }
        ...
        else if (mState == OK)
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
                ...
                {
                    std::cout << "IMU is not or recently initialized. Reseting active std::map..." << std::endl;
                    mpSystem->ResetActiveMap();
                }
...
void Tracking::CreateMapInAtlas()
{
    ...
    mbVelocity = false;
    //mnLastRelocFrameId = mnLastInitFrameId; // The last relocation KF_id is the current id, because it is the new starting point for new std::map
    Verbose::PrintMess("First frame id in std::map: " + to_string(mnLastInitFrameId+1), Verbose::VERBOSITY_NORMAL);
...
bool Tracking::TrackLocalMap()
{
    ...
    // We have an estimation of the camera pose and some std::map points tracked in the frame.
    // We retrieve the local std::map and try to find matches to points in the local std::map.
    mTrackedFr++;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to std::map matches.
...
void Tracking::UpdateLocalKeyFrames()
{
        ...
        for(int i=0; i<mCurrentFrame.N; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    for(std::map<KeyFrame*,tuple<int,int>>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
    ...
    // All keyframes that observe a std::map point are included in the local std::map. Also check which keyframe shares most points
...
{
    unique_lock<std::mutex> lock(mMutexStop);
